...
void Connection::WriteNonBlocking()
{
    ...
    while (data_left > 0)
    {
        ...
        if (bytes_write == -1 && errno == EINTR) // 其他原因导致的中断，继续写
        {
            printf("continue writing\n");
            continue;
        }
        else if (bytes_write == -1 && errno == EAGAIN) // 对方接收buffer满，你发送buffer满
        {
            printf("writing error EAGAIN\n");
            break;
        }
