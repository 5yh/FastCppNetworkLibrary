...
{
    char *start = const_cast<char *>(begin); // 将常量字符指针begin转换为非常量字符指针start
    char *end = start;
    char *colon = end; // keyvalue使用，对于URL:PARAMS 和 HeaderKey: HeaderValue保存中间的位置
    while (state_ != HttpRequestParaseState::kINVALID && state_ != HttpRequestParaseState::COMPLETE && end - begin <= size)
    {
        ...
        case HttpRequestParaseState::PROTOCOL: // http不用保存，保存协议版本
        {
            if (ch == '/')
            {
        ...
        case HttpRequestParaseState::VERSION:
        {
            if (ch == CR)
            {
                ...
                start = end + 1;
                state_ = HttpRequestParaseState::WHEN_CR; // 注意版本后第一行就结束了
            }
    ...
    // 到达complete或者body都是成功，body下一次继续解析
